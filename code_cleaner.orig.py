...
class CodeCleanerApp:
    def __init__(self, root):
        ...
        ttk.Checkbutton(options_frame, text="Remove full-line comments (#...)",
                        variable=self.remove_comments_var).grid(row=0, column=0, padx=6, pady=2, sticky="w")

        self.trim_trailing_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Trim trailing whitespace", variable=self.trim_trailing_var).grid(
            row=0, column=1, padx=6, pady=2, sticky="w")

        self.collapse_blank_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Collapse multiple blank lines",
                        variable=self.collapse_blank_var).grid(row=0, column=2, padx=6, pady=2, sticky="w")

        if HAS_AUTOPEP8:
            self.use_autopep8_var = tk.BooleanVar(value=False)
            ttk.Checkbutton(options_frame, text="Format with autopep8 (if enabled)",
                            variable=self.use_autopep8_var).grid(row=0, column=3, padx=6, pady=2, sticky="w")
        else:
            ttk.Label(options_frame, text="(autopep8 not installed)").grid(
                row=0, column=3, padx=6, pady=2, sticky="w")

        buttons_frame = ttk.Frame(top_frame)
        buttons_frame.pack(side=tk.RIGHT, anchor="e")

        ttk.Button(buttons_frame, text="Swap Input/Output", command=self.swap_io,
                   bootstyle="outline-info").grid(row=0, column=0, padx=6)
        ttk.Button(buttons_frame, text="Clean Code ▶", command=self.clean_code,
                   bootstyle="success").grid(row=0, column=1, padx=6)
        ...
        self.input_text = tk.Text(
            input_frame, wrap="none", undo=True, bg="#ffffff", relief="flat", padx=8, pady=6)
        self._add_scrollbars(input_frame, self.input_text)

        output_frame = ttk.Labelframe(
            paned, text="Cleaned Output", padding=(8, 8))
        output_frame.pack(fill=tk.BOTH, expand=True)
        paned.add(output_frame, weight=1)

        self.output_text = tk.Text(
            output_frame, wrap="none", undo=True, bg="#ffffff", relief="flat", padx=8, pady=6)
        ...
        statusbar = ttk.Label(
            status_frame, textvariable=self.status, anchor=tk.W)
        statusbar.pack(side=tk.LEFT, fill=tk.X,
                       expand=True, padx=(6, 0), pady=4)

        self.progress = ttk.Progressbar(
            status_frame, length=140, mode="determinate")
